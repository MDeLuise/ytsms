...
public class SubscriptionController {
    ...
    @Operation(
        summary = "Create batch of new Subscriptions", description = "Create batch of new Subscriptions."
    )
    @PostMapping("/batch")
    public ResponseEntity<List<SubscriptionDTO>> saveBatch(@RequestBody List<SubscriptionDTO> entityToSave) {
        final Subscription[] toSave =
            entityToSave.stream().map(subscriptionDtoConverter::convertFromDTO).toArray(Subscription[]::new);
        final List<Subscription> result = subscriptionService.saveBatch(toSave);
        return ResponseEntity.ok(result.stream().map(subscriptionDtoConverter::convertToDTO).toList());
    }
}
